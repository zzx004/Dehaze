...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--model', type=str, default='cycle_gan',
                            help='chooses which model to use. [cycle_gan | UCPformer]')
        ...
        parser.add_argument('--netG', type=str, default='UCPformer',
                            help='specify generator architecture [UCPformer]')
        parser.add_argument('--n_layers_D', type=int,
                            default=3, help='only used if netD==n_layers')
        parser.add_argument('--norm', type=str, default='instance',
                            help='instance normalization or batch normalization [instance | batch | none]')
        parser.add_argument('--init_type', type=str, default='pretrain',
                            help='network initialization [pretrain | normal | xavier | kaiming | orthogonal]')
    ...
    def gather_options(self):
        ...
        if not self.initialized:  # check if it has been initialized
            parser = argparse.ArgumentParser(
                formatter_class=argparse.ArgumentDefaultsHelpFormatter)  # 给命令行参数加手册
            parser = self.initialize(parser)

        # get the basic options
        opt, _ = parser.parse_known_args()  # opt是已知参数，_ 是未知参数

        # modify model-related parser options
        model_name = opt.model
        model_option_setter = models.get_option_setter(model_name)
        parser = model_option_setter(parser, self.isTrain)  # 新增或修改模型的parse
        opt, _ = parser.parse_known_args()  # parse again with new defaults

        # modify dataset-related parser options
        dataset_name = opt.dataset_mode
        dataset_option_setter = data.get_option_setter(dataset_name)
        parser = dataset_option_setter(parser, self.isTrain)  # 新增或修改数据集的parse
