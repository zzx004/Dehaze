...
class dataset_single_test(data.Dataset):
    def __init__(self, opts, setname, input_dim):
        ...
        images = os.listdir(os.path.join(
            self.test_path, opts.phase + setname, 'trainA'))
        self.img = [os.path.join(
            self.test_path, opts.phase + setname, 'trainA', x) for x in images]
        self.img.sort()
        self.size = len(self.img)
        self.input_dim = input_dim
        self.img_name = self.img
        transforms1 = [Pad((0, 0, 3, 3), padding_mode='edge'), ToTensor(
        ), Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])]
        self.transforms1 = Compose(transforms1)
        transforms2 = [ToTensor(), Normalize(
...
class dataset_pair(data.Dataset):
    def __init__(self, opts, name, input_dim):
        ...
        self.A = [os.path.join(self.auto_path, name, 'trainA', x)
                  for x in images_A]
        self.A.sort()
        # B
        images_B = os.listdir(os.path.join(self.auto_path, name, 'trainB'))
        self.B = [os.path.join(self.auto_path, name, 'trainB', x)
                  for x in images_B]
        self.B.sort()
...
class dataset_unpair(data.Dataset):
    def __init__(self, opts):
        ...
        self.A = [os.path.join(self.train_path, opts.phase + 'A', x)
                  for x in images_A]
        # B
        images_B = os.listdir(os.path.join(self.train_path, opts.phase + 'B'))
        self.B = [os.path.join(self.train_path, opts.phase + 'B', x)
                  for x in images_B]
...
class dataset_unpair_val(data.Dataset):
    def __init__(self, opts):
        ...
        self.A = [os.path.join(self.val_path, opts.phase + 'A', x)
                  for x in images_A]
        # B
        images_B = os.listdir(os.path.join(self.val_path, opts.phase + 'B'))
        self.B = [os.path.join(self.val_path, opts.phase + 'B', x)
                  for x in images_B]
        ...
        # setup image transformation
        transforms = [CenterCrop(256), ToTensor(), Normalize(
            mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])]
        self.transforms = Compose(transforms)
        print('A: %d, B: %d images' % (self.A_size, self.B_size))
        return
